...
def run_simulation_two_pops(n1: int, n2: int, theta: float, D: float,output_folder):
    ...
    print(plink_command)
    get_linkage = Popen(plink_command.split(),cwd = "first_test")
    ...
    snp_in_file = os.path.join(output_folder, f'plink.prune.out')
    ...
    command = " ".join(['python',location_run_medeas, med_file,label_file,output_folder,str(K),str(1000)])
    print(command)
    medeas = Popen(command.split())
    medeas.communicate()

    output_folder_bis = output_folder + "_bis"
    med_file_bis = os.path.join(output_folder_bis, f'{pattern_snp_file}.med')
    #os.makedirs(output_folder_bis)
    os.system(f"cut -d ' ' -f 5- {snp_file} > {med_file_bis}")
    ...
    command = " ".join(['python',location_run_medeas, med_file_bis,label_file,output_folder_bis,str(K),str(1000)])
    print(command)
    medeas = Popen(command.split())
    medeas.communicate()
